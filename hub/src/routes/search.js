...
module.exports = async function (fastify, opts) {
  ...
  fastify.get('/', async (request, reply) => {
    try {
      ...
      // Search directly using PostgreSQL
      let merchantIds = [];
      
      try {
        // Build dynamic query with JSONB contains operator
        const tagConditions = tagList.map((tag, i) => {
          return `(tags->'categories' ?> $${i + 1} OR tags->'capabilities' ?> $${i + 1} OR tags->>'location' = $${i + 1})`;
        }).join(' OR ');
        ...
        const searchResult = await db.query(queryText, [...tagList, parseInt(limit) * 2]);
